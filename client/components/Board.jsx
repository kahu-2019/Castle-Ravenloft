...
class Board extends Component {
  constructor(props) {
    ...
    this.getTileAndSquareForCharacter = this.getTileAndSquareForCharacter.bind(
      this
    );
  ...
  componentDidMount() {
    ...
    this.setState(
      {
        dataSet: strahdsCrypt,
        cleanTileSet: strahdsCrypt,
        completeTileSet: temp,
        endingPiece
      },
      () => {
  ...
  prepTileForAdding(side) {
    ...
    let tileIndex = Math.floor(
      Math.random() * this.state.completeTileSet.length
    );
  ...
  addTile(tile, position) {
    ...
    if (position.x === 0) {
      ...
      let monster = {};
      monsterFinder: for (let y in tile.grid) {
      ...
      this.state.monsters.push(
        this.getMonster(monster.x, monster.y, this.state.players[0].id)
      );
      ...
      switch (side) {
        case 0:
          if (playerPos.squareY === 0) {
            ...
            for (let item of this.state.cleanTileSet) {
              if (
                item.y === playerPos.tileY - 1 &&
                item.x === playerPos.tileX
              ) {
        ...
        case 1:
          if (playerPos.squareX === 3) {
            ...
            for (let item of this.state.cleanTileSet) {
              if (
                item.y === playerPos.tileY &&
                item.x === playerPos.tileX + 1
              ) {
                tileAlreadyExists = true;
                break;
              }
  ...
  processCharacters() {
    ...
    outerloop: for (let char of this.state.players) {
      let position = this.getTileAndSquareForCharacter(char);
    ...
    outerloop: for (let monster of this.state.monsters) {
    ...
    return (
      <React.Fragment>
        ...
        <div className="board-container">
          <div
            style={{
              ...
              transform: `translate(${this.state.transform.x}px, ${
          ...
          >
            {this.state.dataSet &&
              this.state.dataSet.map((tile, key) => {
                return (
                  <div
                    key={key}
                    style={{
        ...
        <div style={{ position: "absolute", top: "10px", left: "10px" }}>
          ...
          <br />
          {this.state.explore.left && (
            <React.Fragment>
              <button onClick={() => this.prepTileForAdding(3)}>
                Explore left
              </button>
              <br />
            </React.Fragment>
          )}
          {this.state.explore.right && (
            <React.Fragment>
              <button onClick={() => this.prepTileForAdding(1)}>
                Explore right
              </button>
              <br />
            </React.Fragment>
          )}
          {this.state.explore.top && (
            <React.Fragment>
              <button onClick={() => this.prepTileForAdding(0)}>
                Explore top
              </button>
              <br />
            </React.Fragment>
          )}
          {this.state.explore.bottom && (
            <button onClick={() => this.prepTileForAdding(2)}>
              Explore bottom
            </button>
          )}
        </div>
